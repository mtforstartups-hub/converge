...
export default function IntelligencePage() {
  return (
    <div className="min-h-screen relative overflow-hidden">
      {/* Hero Section */}
      <section className="pt-20 pb-10 md:pt-30 md:pb-16 bg-primary border-b border-white/5">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                <div className="space-y-4">
                  ...
                  <p className="text-base md:text-lg text-font font-semibold">
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-sm md:text-base text-accent font-medium italic">
              ...
              {comparisons.map((item, index) => (
                <div
                  ...
                  className={`p-8 rounded-2xl transition-all duration-500 flex flex-col justify-between h-full ${
                    item.highlight
                      ? "bg-white shadow-[0_20px_50px_rgba(0,0,0,0.1)] ring-1 ring-accent/20 z-10  border-t-accent border-t-[3px]"
                      : "bg-deep-slate border border-primary/5 hover:bg-deep-slate/90"
                  }`}
                >
                  <div className="space-y-6">
                    <h3
                      className={`text-xl font-bold ${item.highlight ? "text-accent" : "text-font"}`}
                    >
                      {item.title}
                    </h3>
                    <ul className="space-y-4">
                      {item.features.map((feature, fIndex) => (
                        <li key={fIndex} className="flex gap-3">
                          <div
                            className={`mt-1.5 size-1.5 rounded-full shrink-0 ${item.highlight ? "bg-accent" : "bg-font/20"}`}
                          />
                          <span
                            className={`text-sm md:text-base leading-relaxed ${item.highlight ? "text-primary/90 font-medium" : "text-font/60"}`}
                          >
                            {feature}
                          </span>
                        </li>
        ...
        <div className="mx-auto max-w-95/100 md:max-w-5/6 px-4 md:px-6 lg:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-16 items-start">
            <div className="lg:col-span-5 space-y-6">
              <h2 className="text-3xl md:text-5xl font-bold text-font tracking-tight leading-tight">
            ...
            <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
              {workStreams.map((stream, index) => (
                <div
                  key={index}
                  className="flex items-center gap-4 py-4 border-b border-white/5 group hover:border-accent/30 transition-colors"
                >
                  <span className="text-[10px] font-mono text-accent/50 group-hover:text-accent">
                    {(index + 1).toString().padStart(2, "0")}
                  </span>
                  <p className="text-sm md:text-base text-white/80 font-medium group-hover:text-font transition-colors">
                    {stream}
                  </p>
                </div>
              ))}
      ...
      <section className="py-10 md:py-20 bg-parchment relative overflow-hidden">
        ...
        <div className="mx-auto max-w-95/100 md:max-w-5/6 px-4 md:px-6 lg:px-10 text-center relative z-10">
          <div className="max-w-3xl mx-auto space-y-8">
            <h2 className="text-xl md:text-3xl lg:text-4xl text-primary font-bold leading-tight">
